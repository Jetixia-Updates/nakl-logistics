import Link from 'next/link';
import { 
  Truck, 
  Package, 
  MapPin, 
  Clock, 
  Shield, 
  Users,
  TrendingUp,
  Award,
  Phone,
  Mail,
  MapPinned,
  ArrowRight,
  CheckCircle
} from 'lucide-react';

export default function Home() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      {/* Header */}
      <header className="bg-white shadow-sm sticky top-0 z-50">
        <nav className="container mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <Truck className="h-8 w-8 text-primary" />
              <span className="text-2xl font-bold text-primary">نقل</span>
            </div>
            
            <div className="hidden md:flex gap-8">
              <a href="#services" className="text-gray-700 hover:text-primary transition">الخدمات</a>
              <a href="#about" className="text-gray-700 hover:text-primary transition">من نحن</a>
              <a href="#features" className="text-gray-700 hover:text-primary transition">المميزات</a>
              <a href="#contact" className="text-gray-700 hover:text-primary transition">اتصل بنا</a>
            </div>
            
            <div className="flex gap-4">
              <Link 
                href="/login"
                className="px-6 py-2 text-primary border border-primary rounded-lg hover:bg-primary hover:text-white transition"
              >
                تسجيل الدخول
              </Link>
              <button className="text-gray-600 hover:text-primary transition">
                EN
              </button>
            </div>
          </div>
        </nav>
      </header>

      {/* Hero Section */}
      <section className="container mx-auto px-6 py-20">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            <h1 className="text-5xl font-bold text-gray-900 mb-6 leading-tight">
              حلول نقل وشحن <span className="text-primary">احترافية</span> لأعمالك
            </h1>
            <p className="text-xl text-gray-600 mb-8">
              نوفر خدمات النقل والشحن واللوجستيات المتكاملة بأعلى معايير الجودة والأمان في جميع أنحاء مصر
            </p>
            <div className="flex gap-4">
              <Link 
                href="/track"
                className="px-8 py-4 bg-primary text-white rounded-lg hover:bg-primary/90 transition flex items-center gap-2 text-lg font-medium"
              >
                تتبع الشحنة
                <ArrowRight className="h-5 w-5" />
              </Link>
              <a 
                href="#contact"
                className="px-8 py-4 border-2 border-primary text-primary rounded-lg hover:bg-primary hover:text-white transition text-lg font-medium"
              >
                اطلب عرض سعر
              </a>
            </div>
          </div>
          
          <div className="relative">
            <div className="absolute top-10 right-10 w-72 h-72 bg-primary/10 rounded-full blur-3xl"></div>
            <div className="relative bg-white rounded-2xl shadow-2xl p-8">
              <div className="flex items-center gap-4 mb-6">
                <div className="p-3 bg-primary/10 rounded-lg">
                  <Truck className="h-8 w-8 text-primary" />
                </div>
                <div>
                  <div className="text-3xl font-bold text-gray-900">500+</div>
                  <div className="text-gray-600">عملية نقل شهرياً</div>
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <CheckCircle className="h-5 w-5 text-green-500" />
                  <span className="text-gray-700">تتبع لحظي بنظام GPS</span>
                </div>
                <div className="flex items-center gap-3">
                  <CheckCircle className="h-5 w-5 text-green-500" />
                  <span className="text-gray-700">أسطول شاحنات حديث</span>
                </div>
                <div className="flex items-center gap-3">
                  <CheckCircle className="h-5 w-5 text-green-500" />
                  <span className="text-gray-700">تأمين شامل على البضائع</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Services Section */}
      <section id="services" className="bg-gray-50 py-20">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-gray-900 mb-4">خدماتنا المتكاملة</h2>
            <p className="text-xl text-gray-600">نقدم حلول نقل شاملة تلبي جميع احتياجات عملك</p>
          </div>
          
          <div className="grid md:grid-cols-3 gap-8">
            <ServiceCard 
              icon={<Truck className="h-12 w-12 text-primary" />}
              title="النقل البري"
              description="خدمات نقل برية متكاملة بأسطول حديث ومتنوع يشمل جميع أنواع الشاحنات"
            />
            <ServiceCard 
              icon={<Package className="h-12 w-12 text-primary" />}
              title="التخزين والتوزيع"
              description="مستودعات حديثة مجهزة بأنظمة إدارة متقدمة لضمان سلامة البضائع"
            />
            <ServiceCard 
              icon={<MapPin className="h-12 w-12 text-primary" />}
              title="التتبع اللحظي"
              description="نظام تتبع متطور بتقنية GPS لمعرفة موقع شحنتك في أي وقت"
            />
            <ServiceCard 
              icon={<Clock className="h-12 w-12 text-primary" />}
              title="التسليم السريع"
              description="خدمة توصيل سريعة مع التزام تام بمواعيد التسليم المحددة"
            />
            <ServiceCard 
              icon={<Shield className="h-12 w-12 text-primary" />}
              title="التأمين الشامل"
              description="تأمين كامل على جميع الشحنات لضمان راحة بالك"
            />
            <ServiceCard 
              icon={<Users className="h-12 w-12 text-primary" />}
              title="دعم العملاء"
              description="فريق دعم محترف متاح 24/7 للرد على استفساراتك"
            />
          </div>
        </div>
      </section>

      {/* Features Section */}
      <section id="features" className="py-20">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-gray-900 mb-4">لماذا تختار نقل؟</h2>
            <p className="text-xl text-gray-600">نتميز بالخبرة والاحترافية والتزام بأعلى معايير الجودة</p>
          </div>
          
          <div className="grid md:grid-cols-4 gap-6">
            <FeatureCard 
              icon={<TrendingUp className="h-8 w-8 text-primary" />}
              number="15+"
              label="سنة خبرة"
            />
            <FeatureCard 
              icon={<Truck className="h-8 w-8 text-primary" />}
              number="200+"
              label="شاحنة"
            />
            <FeatureCard 
              icon={<Users className="h-8 w-8 text-primary" />}
              number="500+"
              label="عميل راضٍ"
            />
            <FeatureCard 
              icon={<Award className="h-8 w-8 text-primary" />}
              number="98%"
              label="معدل الرضا"
            />
          </div>
        </div>
      </section>

      {/* About Section */}
      <section id="about" className="bg-primary text-white py-20">
        <div className="container mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-4xl font-bold mb-6">من نحن</h2>
              <p className="text-lg mb-4 opacity-90">
                شركة نقل هي إحدى الشركات الرائدة في مجال النقل والخدمات اللوجستية في مصر. 
                نعمل منذ أكثر من 15 عاماً على تقديم حلول نقل متكاملة تلبي احتياجات مختلف القطاعات.
              </p>
              <p className="text-lg mb-4 opacity-90">
                نفخر بامتلاكنا أسطول حديث من الشاحنات ونظام إدارة متطور يضمن تتبع الشحنات 
                لحظياً وتسليمها في الوقت المحدد بأعلى معايير الأمان.
              </p>
              <p className="text-lg opacity-90">
                رؤيتنا هي أن نكون الخيار الأول لخدمات النقل في مصر والشرق الأوسط، 
                من خلال الابتكار المستمر والالتزام بأعلى معايير الجودة.
              </p>
            </div>
            
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8">
              <h3 className="text-2xl font-bold mb-6">قيمنا</h3>
              <div className="space-y-4">
                <ValueItem title="الاحترافية" description="نلتزم بأعلى معايير الاحترافية في كل ما نقوم به" />
                <ValueItem title="الأمانة" description="نحافظ على ثقة عملائنا من خلال الشفافية والنزاهة" />
                <ValueItem title="الجودة" description="نسعى دائماً لتقديم خدمات بأعلى مستوى من الجودة" />
                <ValueItem title="الابتكار" description="نستخدم أحدث التقنيات لتطوير خدماتنا" />
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Contact Section */}
      <section id="contact" className="py-20">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-gray-900 mb-4">تواصل معنا</h2>
            <p className="text-xl text-gray-600">نحن هنا للإجابة على استفساراتك وخدمتك</p>
          </div>
          
          <div className="grid md:grid-cols-3 gap-8 mb-12">
            <ContactCard 
              icon={<Phone className="h-8 w-8 text-primary" />}
              title="الهاتف"
              content="+20 2 1234 5678"
            />
            <ContactCard 
              icon={<Mail className="h-8 w-8 text-primary" />}
              title="البريد الإلكتروني"
              content="info@nakl.com"
            />
            <ContactCard 
              icon={<MapPinned className="h-8 w-8 text-primary" />}
              title="العنوان"
              content="القاهرة، مصر"
            />
          </div>
          
          <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl p-8">
            <form className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <input
                  type="text"
                  placeholder="الاسم"
                  className="px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                />
                <input
                  type="email"
                  placeholder="البريد الإلكتروني"
                  className="px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                />
              </div>
              <input
                type="text"
                placeholder="الموضوع"
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
              />
              <textarea
                rows={5}
                placeholder="رسالتك"
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
              ></textarea>
              <button
                type="submit"
                className="w-full px-8 py-4 bg-primary text-white rounded-lg hover:bg-primary/90 transition font-medium text-lg"
              >
                إرسال الرسالة
              </button>
            </form>
          </div>
        </div>
      </section>

      {/* Footer */}
      <footer className="bg-gray-900 text-white py-12">
        <div className="container mx-auto px-6">
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <Truck className="h-8 w-8 text-primary" />
                <span className="text-2xl font-bold">نقل</span>
              </div>
              <p className="text-gray-400">
                حلول نقل وشحن احترافية لأعمالك
              </p>
            </div>
            
            <div>
              <h4 className="font-bold text-lg mb-4">خدماتنا</h4>
              <ul className="space-y-2 text-gray-400">
                <li><a href="#" className="hover:text-white transition">النقل البري</a></li>
                <li><a href="#" className="hover:text-white transition">التخزين</a></li>
                <li><a href="#" className="hover:text-white transition">التوزيع</a></li>
                <li><a href="#" className="hover:text-white transition">التتبع</a></li>
              </ul>
            </div>
            
            <div>
              <h4 className="font-bold text-lg mb-4">روابط سريعة</h4>
              <ul className="space-y-2 text-gray-400">
                <li><a href="#" className="hover:text-white transition">من نحن</a></li>
                <li><a href="#" className="hover:text-white transition">الخدمات</a></li>
                <li><a href="#" className="hover:text-white transition">المميزات</a></li>
                <li><a href="#" className="hover:text-white transition">اتصل بنا</a></li>
              </ul>
            </div>
            
            <div>
              <h4 className="font-bold text-lg mb-4">تابعنا</h4>
              <div className="flex gap-4">
                <a href="#" className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-primary transition">
                  f
                </a>
                <a href="#" className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-primary transition">
                  t
                </a>
                <a href="#" className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-primary transition">
                  in
                </a>
              </div>
            </div>
          </div>
          
          <div className="border-t border-gray-800 pt-8 text-center text-gray-400">
            <p>© 2024 نقل. جميع الحقوق محفوظة.</p>
          </div>
        </div>
      </footer>
    </div>
  );
}

// Helper Components
function ServiceCard({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <div className="bg-white rounded-xl p-8 shadow-lg hover:shadow-xl transition">
      <div className="mb-4">{icon}</div>
      <h3 className="text-xl font-bold text-gray-900 mb-3">{title}</h3>
      <p className="text-gray-600">{description}</p>
    </div>
  );
}

function FeatureCard({ icon, number, label }: { icon: React.ReactNode; number: string; label: string }) {
  return (
    <div className="bg-white rounded-xl p-6 shadow-lg text-center">
      <div className="flex justify-center mb-4">{icon}</div>
      <div className="text-3xl font-bold text-gray-900 mb-2">{number}</div>
      <div className="text-gray-600">{label}</div>
    </div>
  );
}

function ValueItem({ title, description }: { title: string; description: string }) {
  return (
    <div className="flex gap-3">
      <CheckCircle className="h-6 w-6 flex-shrink-0 mt-0.5" />
      <div>
        <div className="font-semibold mb-1">{title}</div>
        <div className="text-sm opacity-80">{description}</div>
      </div>
    </div>
  );
}

function ContactCard({ icon, title, content }: { icon: React.ReactNode; title: string; content: string }) {
  return (
    <div className="bg-white rounded-xl p-6 shadow-lg text-center">
      <div className="flex justify-center mb-4">{icon}</div>
      <h3 className="font-bold text-lg text-gray-900 mb-2">{title}</h3>
      <p className="text-gray-600">{content}</p>
    </div>
  );
}
