...
export default function VehiclesPage() {
  ...
  const vehicles = [
    ...
    },
    {
      ...
      currentLocation: language === 'ar' ? 'ورشة الصيانة - القاهرة' : 'Maintenance Workshop - Cairo',
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {language === 'ar' ? 'إدارة أسطول المركبات والسائقين ومراقبة الأداء' : 'Manage vehicle fleet, drivers, and monitor performance'}
          </p>
        </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className={`${language === 'ar' ? 'mr-4' : 'ml-4'}`}>
                ...
                <p className="text-sm text-gray-600">{language === 'ar' ? 'في الطريق' : 'In Transit'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className={`${language === 'ar' ? 'mr-4' : 'ml-4'}`}>
                ...
                <p className="text-sm text-gray-600">{language === 'ar' ? 'متوسط التقييم' : 'Average Rating'}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 flex-1">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <option value="in-transit">{language === 'ar' ? 'في الطريق' : 'In Transit'}</option>
              ...
              <div className="p-6">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{vehicle.plateNumber} • {vehicle.year}</p>
                  </div>
                ...
                <div className="space-y-3 mb-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">{language === 'ar' ? 'الحمولة:' : 'Capacity:'}</span>
                    <span className="text-sm font-medium text-gray-900">{vehicle.capacity}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">{language === 'ar' ? 'المسافة المقطوعة:' : 'Mileage:'}</span>
                    <span className="text-sm font-medium text-gray-900">{vehicle.mileage.toLocaleString()} {language === 'ar' ? 'كم' : 'km'}</span>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600">{language === 'ar' ? 'مستوى الوقود' : 'Fuel Level'}</span>
                    </div>
                ...
                <div className="space-y-2 mb-4 text-xs">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">{language === 'ar' ? 'الصيانة القادمة:' : 'Next Service:'}</span>
                    <span className="text-gray-900">{vehicle.nextService}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">{language === 'ar' ? 'التأمين:' : 'Insurance:'}</span>
                    <span className="text-gray-900">{vehicle.insurance}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">{language === 'ar' ? 'الترخيص:' : 'License:'}</span>
                ...
                <div className="flex items-center justify-between mb-4 p-2 bg-blue-50 rounded">
                  <span className="text-sm text-blue-800">{language === 'ar' ? 'الأرباح هذا الشهر:' : 'This Month Earnings:'}</span>
                  <span className="text-sm font-bold text-blue-800">{vehicle.earnings.toLocaleString()} {language === 'ar' ? 'ج.م' : 'EGP'}</span>
                </div>
