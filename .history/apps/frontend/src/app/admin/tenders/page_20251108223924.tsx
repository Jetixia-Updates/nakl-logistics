...
export default function TendersPage() {
  ...
  const tenders = [
    {
      id: 'TND-001',
      title: language === 'ar' ? 'نقل بضائع من القاهرة إلى الإسكندرية' : 'Cargo Transport Cairo to Alexandria',
      value: '25,000',
      currency: 'EGP',
      deadline: '2024-12-15',
      status: 'active',
      bidsCount: 12,
      description: language === 'ar' ? 'نقل 500 طن من البضائع المتنوعة' : 'Transport 500 tons of mixed cargo',
      createdDate: '2024-11-01'
    },
    {
      id: 'TND-002',
      title: language === 'ar' ? 'خدمات التخليص الجمركي - ميناء السخنة' : 'Customs Clearance Services - Sokhna Port',
      value: '15,000',
      currency: 'EGP',
      deadline: '2024-12-20',
      status: 'pending',
      bidsCount: 8,
      description: language === 'ar' ? 'تخليص جمركي لحاويات متعددة' : 'Customs clearance for multiple containers',
    ...
    },
    {
      ...
      status: 'closed',
      bidsCount: 15,
      description: language === 'ar' ? 'نقل آمن للمواد الكيميائية مع التراخيص اللازمة' : 'Safe transport of chemicals with required licenses',
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {language === 'ar' ? 'إدارة المناقصات والعطاءات وتتبع العروض المقدمة' : 'Manage tenders, bids, and track submitted proposals'}
          </p>
        </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className={`${language === 'ar' ? 'mr-4' : 'ml-4'}`}>
                ...
                <p className="text-sm text-gray-600">{language === 'ar' ? 'إجمالي المناقصات' : 'Total Tenders'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className={`${language === 'ar' ? 'mr-4' : 'ml-4'}`}>
                ...
                <p className="text-sm text-gray-600">{language === 'ar' ? 'إجمالي العروض' : 'Total Bids'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className={`${language === 'ar' ? 'mr-4' : 'ml-4'}`}>
                ...
                <p className="text-sm text-gray-600">{language === 'ar' ? 'القيمة الإجمالية (ج.م)' : 'Total Value (EGP)'}</p>
              </div>
            </div>
          </div>
        </div>
                ...
                {filteredTenders.map((tender) => (
                  <tr key={tender.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        <button className="text-blue-600 hover:text-blue-900" aria-label={language === 'ar' ? 'عرض التفاصيل' : 'View Details'}>
                          <Eye className="w-4 h-4" />
                        </button>
                        <button className="text-green-600 hover:text-green-900" aria-label={language === 'ar' ? 'تحرير' : 'Edit'}>
                          <Edit className="w-4 h-4" />
                        </button>
                        <button className="text-red-600 hover:text-red-900" aria-label={language === 'ar' ? 'حذف' : 'Delete'}>
