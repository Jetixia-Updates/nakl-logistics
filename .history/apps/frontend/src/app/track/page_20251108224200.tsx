...
export default function TrackingPage() {
    ...
    'TRK002': {
      ...
      timeline: [
        ...
        },
        {
          ...
          description: language === 'ar' ? 'في الطريق - نقطة تفتيش القاهرة' : 'In transit - Cairo checkpoint'
        },
        {
          status: 'in-transit',
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'in-transit': return <Truck className="w-5 h-5 text-blue-600" />;
      case 'picked-up': return <Package className="w-5 h-5 text-orange-600" />;
      case 'arrived': return <MapPin className="w-5 h-5 text-purple-600" />;
      default: return <Clock className="w-5 h-5 text-gray-600" />;
    }
  };

  return (
    <div className={`min-h-screen bg-gradient-to-b from-blue-50 to-white ${language === 'ar' ? 'font-arabic' : 'font-inter'}`}>
        ...
        <div className="max-w-2xl mx-auto mb-12">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1">
                ...
                <input
                  id="tracking"
                  type="text"
                  placeholder={language === 'ar' ? 'أدخل رقم التتبع (مثال: TRK001)' : 'Enter tracking number (e.g., TRK001)'}
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-600 mb-2">
              {language === 'ar' ? 'جرب أرقام التتبع التالية:' : 'Try these sample tracking numbers:'}
            </p>
            ...
            <div className="bg-white rounded-2xl shadow-lg border mb-8 overflow-hidden">
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-6 text-white">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`px-4 py-2 rounded-full ${getStatusColor(trackingResult.status)}`}>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Customer Info */}
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-4">{language === 'ar' ? 'معلومات العميل' : 'Customer Information'}</h3>
                  ...
                  {/* Dates */}
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-4">{language === 'ar' ? 'التواريخ المهمة' : 'Important Dates'}</h3>
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <Calendar className="w-4 h-4 text-gray-400" />
                        <div>
                          <span className="text-sm text-gray-600">{language === 'ar' ? 'تاريخ الشحن:' : 'Shipped:'}</span>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-sm text-gray-600">{language === 'ar' ? 'التسليم المتوقع:' : 'Expected Delivery:'}</span>
                          <span className="text-gray-900 ml-2">{trackingResult.estimatedDelivery}</span>
                        </div>
                      </div>
                      {trackingResult.actualDelivery && (
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm text-gray-600">{language === 'ar' ? 'تم التسليم:' : 'Delivered:'}</span>
                            <span className="text-gray-900 ml-2">{trackingResult.actualDelivery}</span>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  {/* Route */}
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-4">{language === 'ar' ? 'المسار' : 'Route'}</h3>
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <MapPin className="w-4 h-4 text-green-500" />
                        <div>
                          <span className="text-sm text-gray-600">{language === 'ar' ? 'من:' : 'From:'}</span>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-sm text-gray-600">{language === 'ar' ? 'إلى:' : 'To:'}</span>
                  ...
                  {/* Transport Details */}
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-4">{language === 'ar' ? 'تفاصيل النقل' : 'Transport Details'}</h3>
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <Truck className="w-4 h-4 text-gray-400" />
                        <div>
                          <span className="text-sm text-gray-600">{language === 'ar' ? 'المركبة:' : 'Vehicle:'}</span>
                      ...
                      <div className="flex items-center gap-3">
                        <Package className="w-4 h-4 text-gray-400" />
                        <div>
                          <span className="text-sm text-gray-600">{language === 'ar' ? 'البضاعة:' : 'Cargo:'}</span>
                    ...
                    <div key={index} className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <h4 className="font-semibold text-gray-900">{getStatusText(event.status)}</h4>
